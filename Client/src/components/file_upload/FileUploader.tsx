  ...
  function handleFileChange(e: ChangeEvent<HTMLInputElement>) {
    if (e.target.files && e.target.files.length > 0) {
      const newFiles = Array.from(e.target.files).map(file => {
        ...
        return {
          file,
          status: validation.isValid ? 'idle' as UploadStatus : 'error' as UploadStatus,
          progress: 0,
          ...
          <div className="relative">
            ...
            >
              ...
              <span className="text-sm font-medium text-gray-600">
                {files.length > 0 ? 'Add more files' : 'Drag and drop your files here'}
              </span>
              <span className="text-xs text-gray-500">
                PDF, PNG, JPG, TXT only (up to 10MB)
              </span>
            </button>
          </div>
          ...
          {files.length > 0 && (
            ...
            <div className="space-y-3">
              ...
              {files.map((fileWithStatus, index) => (
                <div key={fileWithStatus.file.name + index} 
                     className={`text-sm border rounded-md p-3 ${
                       fileWithStatus.error ? 'border-red-200 bg-red-50' : 'border-gray-200 bg-gray-50'
                     } relative`}>
