...
export default function FileUploader() {
  ...
  const handleApprove = async () => {
    ...
    for (let i = 0; i < rowCount; i++) {
      ...
      sellingPrice: tableData['Selling Price'][i], // Ensure the key matches the schema
      discount: tableData['Discount'][i],
      customer: tableData['Customer'][i],
    };

    // Validate each row to ensure it aligns with the schema
    if (!row.UserId || !row.date || !row.item || !row.sellingPrice || !row.discount || !row.customer) {
    ...
    try {
      await axios.post('http://localhost:5001/api/transactions/store', { rows },{withCredentials:true});
          ...
          <div className="relative">
            <ul className="w-full flex flex-col items-center justify-center text-gray-600">
              ...
              <li className="flex items-center text-center justify-center text-sm text-gray-600">
                1. Make sure that there is enough contrast between the image and
                the background for better visibility.
              </li>
              <li className="flex items-center text-center justify-center text-sm text-gray-600">
                2. Check the ledger generated by the system to ensure that the
                data is accurate and complete before committing to the database.
              </li>
              <li className="flex items-center text-center justify-center text-sm text-gray-600">
              ...
              {files.map((fileWithStatus, index) => (
                ...
                >
                  ...
                  <p className="text-gray-600 text-xs">
                    Size: {(fileWithStatus.file.size / 1024).toFixed(2)} KB
                  </p>
                  {fileWithStatus.error && (
                  ...
                  {fileWithStatus.status === 'uploading' && (
                    <div className="mt-2 space-y-1">
                      ...
                      <p className="text-xs text-blue-600 text-right">
                        {fileWithStatus.progress}% uploaded
                      </p>
                    </div>
                  )}
                    ...
                    {getTableRows().map((row, rowIdx) => (
                      <tr key={rowIdx}>
                        {Object.entries(row).map(([column, value]) => (
                          <td key={column} className="border px-2 py-1">
                            <input
                              className="w-full border rounded px-1 py-0.5 text-xs"
                              value={value}
                              onChange={(e) =>
                                handleTableEdit(rowIdx, column, e.target.value)
                              }
                            />
                          </td>
                        ))}
